...
def render_hero(rd: dict, source_label: str) -> None:
    name = _coalesce(rd.get("name"), "SCOPE")
    ...
    st.markdown(
        f"""
<div class="mp-hero">
  <span class="eyebrow">Source Credibility and Outlet Profiling Engine</span>
  <h1>{_esc(name)}</h1>
...
def render_system_overview(summary: dict) -> None:
    ...
    st.markdown(
        ...
        "<p><strong>SCOPE</strong> is an automated pipeline for outlet-level "
